Feature: Initialize a new Forge project
  ...
  Scenario: Creating a brand new project with all arguments
    When I successfully run `forge init foo --name=foo_theme --uri=http://www.footheme.com --author="Foo Man" --author_uri=http://www.fooman.com`
  ...
  Scenario: Creating a new project without any arguments
    When I run `forge init foo` interactively
